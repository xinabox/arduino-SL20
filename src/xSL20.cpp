...
void xSL20::measure(bool calculate_temps) {
	if (checkConfig()) {
		readEEPROM();
		writeTrimmingValue();
		setConfiguration();
...
float xSL20::getTemperature(int num) {
	if ((num >= 0) && (num < 64)) {
...
float xSL20::getAmbient() {
	return Tambient;
}

void xSL20::setConfiguration() {
	byte Hz_LSB;
	switch (refreshRate) {
	case 0:
		Hz_LSB = 0b00111111;
...
void xSL20::readEEPROM() { // Read in blocks of 32 bytes to accomodate Wire library
...
void xSL20::calculateTA(void) {
	Tambient = ((-k_t1 + sqrt(sq(k_t1) - (4 * k_t2 * (v_th - (float) ptat))))
			/ (2 * k_t2)) + 25.0;
}

void xSL20::preCalculateConstants() {
...
float xSL20::getMinTemp() {
	return minTemp;
}

float xSL20::getMaxTemp() {
	return maxTemp;
}
...
int16_t xSL20::twos_16(uint8_t highByte, uint8_t lowByte){
...
int8_t xSL20::twos_8(uint8_t byte) {
	if (byte > 127)
		return (int8_t) byte - 256;
	return (int8_t) byte;
}

uint16_t xSL20::unsigned_16(uint8_t highByte, uint8_t lowByte){
	return (highByte << 8) | lowByte;
}

uint16_t xSL20::readConfig() {
	Wire.beginTransmission(0x60);
	Wire.write(0x02);
	Wire.write(0x92);
	Wire.write(0x00);
